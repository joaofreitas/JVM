...
public class Belote {
	...
	public static String retornaNome(char naipe){
		if(naipe=='0'){
			return("Paus");
		}
		else if(naipe=='1'){
	...
	public Belote(){
		...
		for( int i=0;i<4;i++){
			for(int j=0;j<13;j++){
				baralho[i*13+j] = new Carta((char)(i+48),cartaValores[j],j+1);
				System.out.println(baralho[i].nome);
				//baralho[i].printOut();
				//System.out.println("#########################################################################################");
//				for( int k=0;k<=i;k++){
//					for(int l=0;l<=j;l++){				
//						System.out.println(baralho[k*13+l].naipe);
//						System.out.println(baralho[k*13+l].nome);
//					}
//				}
			}
		}
